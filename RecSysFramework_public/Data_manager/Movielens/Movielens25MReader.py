...
class Movielens25MReader(DataReader):
    ...
    def _load_from_original_file(self):
        ...
        try:
            ...
            dataFile = zipfile.ZipFile(zipFile_path + "ml-25m.zip")

        except (FileNotFoundError, zipfile.BadZipFile):

            self._print("Unable to find data zip file. Downloading...")

            download_from_URL(self.DATASET_URL, zipFile_path, "ml-25m.zip")

            dataFile = zipfile.ZipFile(zipFile_path + "ml-25m.zip")


        ICM_genre_path = dataFile.extract("ml-25m/movies.csv", path=zipFile_path + "decompressed/")
        ICM_tags_path = dataFile.extract("ml-25m/tags.csv", path=zipFile_path + "decompressed/")
        URM_path = dataFile.extract("ml-25m/ratings.csv", path=zipFile_path + "decompressed/")
