...
DICT_SEARCH_CONFIGS = {
    ...
    "SIGNAL_ANALYSIS_NEGATIVE": SearchHyperParametersWeightedUserProfileRecommender(
        sign=Categorical(categories=[-1]),
    ),
    "SIGNAL_ANALYSIS_POSITIVE": SearchHyperParametersWeightedUserProfileRecommender(
        sign=Categorical(categories=[1]),
    ),
}
...
class ItemWeightedUserProfileRecommender(BaseWeightedUserProfileRecommender):
    ...
    def _compute_item_score(
        ...
        if sp.issparse(item_scores_all):
            item_scores_all = item_scores_all.toarray()
