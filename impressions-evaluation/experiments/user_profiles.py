...
from impression_recommenders.user_profile.weighted import DICT_SEARCH_CONFIGS as \
    ImpressionsAsUserProfilesSearchConfigs
...
def _run_signal_analysis_impressions_user_profiles_hyper_parameter_tuning(
    experiment_case_user_profile: commons.ExperimentCase,
    experiment_case_baseline: commons.ExperimentCase,
    experiment_baseline_similarity: Optional[str],
    ...
    """
    Runs in a dask worker the hyper-parameter tuning of an impression as user profiles recommender by manually
    selecting the signal indicated in `signal_analysis_type`.
    ...
    for try_folded_recommender in [True, False]:
        ...
        requires_user_similarity = issubclass(experiment_user_profiles_recommender.recommender,
                                              UserWeightedUserProfileRecommender)
        requires_item_similarity = issubclass(experiment_user_profiles_recommender.recommender,
                                              ItemWeightedUserProfileRecommender)
        ...
        experiment_file_name_root = (
            f"{signal_analysis_type.value}"
            f"_{experiment_user_profiles_recommender.recommender.RECOMMENDER_NAME}"
            f"_{baseline_recommender_trained_train.RECOMMENDER_NAME}"
        )
        ...
        hyper_parameter_search_space = attrs.asdict(
            ImpressionsAsUserProfilesSearchConfigs[signal_analysis_type.value],
        )
        ...
        for experiment_case_baseline in baseline_experiment_cases_interface.experiment_cases:
            ...
            for similarity in similarities:
                ...
                for signal_analysis_type in commons.SignalAnalysisType:
                    dask_interface.submit_job(
                        job_key=(
                            f"_run_signal_analysis_impressions_user_profiles_hyper_parameter_tuning"
                            f"|{user_profiles_benchmark.benchmark.value}"
                            f"|{user_profiles_recommender.recommender.RECOMMENDER_NAME}"
                            f"|{baseline_recommender.recommender.RECOMMENDER_NAME}"
                            f"|{similarity}"
                            f"|{signal_analysis_type}"
                            f"|{uuid.uuid4()}"
                        ),
                        job_priority=(
                                user_profiles_benchmark.priority
                        ...
                        method=_run_signal_analysis_impressions_user_profiles_hyper_parameter_tuning,
                        method_kwargs={
                            "experiment_case_user_profile": experiment_case_user_profiles,
                            "experiment_case_baseline": experiment_case_baseline,
                            "experiment_baseline_similarity": similarity,
