...
ARTICLE_BASELINES: list[Type[BaseRecommender]] = [
    ...
    recommenders.P3alphaRecommender,
...
def _print_signal_analysis_impressions_user_profiles_metrics(
    baseline_experiment_cases_interface: commons.ExperimentCasesInterface,
    user_profiles_experiment_cases_interface: commons.ExperimentCasesInterface,
    baseline_experiment_benchmark: commons.ExperimentBenchmark,
    baseline_experiment_hyper_parameters: commons.HyperParameterTuningParameters,
        ...
        for baseline_experiment_recommender in baseline_experiment_recommenders:
            ...
            for similarity in similarities:
                for try_folded_recommender in [True, False]:
                    ...
                    positive_user_profiles_recommender = user_profiles_class(
                        urm_train=interaction_data_splits.sp_urm_train_validation,
                        uim_train=sp.csr_matrix([[]]),
                        trained_recommender=loaded_baseline_recommender,
                    )
                    negative_user_profiles_recommender = user_profiles_class(
                        urm_train=interaction_data_splits.sp_urm_train_validation,
                        uim_train=sp.csr_matrix([[]]),
                        trained_recommender=loaded_baseline_recommender,
                    )

                    positive_user_profiles_recommender.RECOMMENDER_NAME = (
                        f"{commons.SignalAnalysisType.POSITIVE.value}"
                        f"_{positive_user_profiles_recommender.RECOMMENDER_NAME}"
                    )
                    negative_user_profiles_recommender.RECOMMENDER_NAME = (
                        f"{commons.SignalAnalysisType.NEGATIVE.value}"
                        f"_{negative_user_profiles_recommender.RECOMMENDER_NAME}"
                    )

                    base_algorithm_list.append(positive_user_profiles_recommender)
                    base_algorithm_list.append(negative_user_profiles_recommender)
...
def _results_to_pandas(
    list_df_results: list[pd.DataFrame],
    results_name: str,
    ...
    df_results[MODEL_COLUMN] = (
        df_results[MODEL_COLUMN]
            ...
            .str.replace("SIGNAL ANALYSIS POSITIVE", "SA Positive")
            .str.replace("SIGNAL ANALYSIS NEGATIVE", "SA Negative")
            .str.strip()
    )
    ...
    df_results[MODEL_BASE_COLUMN] = (
        df_results[MODEL_COLUMN]
            ...
            .str.replace("SIGNAL ANALYSIS POSITIVE", "")
            .str.replace("SIGNAL ANALYSIS NEGATIVE", "")
            .str.replace("SA Positive", "")
            .str.replace("SA Negative", "")
            .str.strip()
    )
...
def print_results(
    ...
    for benchmark, hyper_parameters in itertools.product(baseline_benchmarks, baseline_hyper_parameters):
        ...
        results_signal_analysis_user_profiles = _print_signal_analysis_impressions_user_profiles_metrics(
            user_profiles_experiment_cases_interface=user_profiles_experiment_cases_interface,
            baseline_experiment_cases_interface=baseline_experiment_cases_interface,
            baseline_experiment_hyper_parameters=experiment_hyper_parameters,
            baseline_experiment_benchmark=experiment_benchmark,
        ...
        _results_to_pandas(
            list_df_results=[
                ...
                results_signal_analysis_user_profiles.df_results,
            ],
        ...
        _results_to_pandas(
            list_df_results=[
                ...
                results_signal_analysis_user_profiles.df_times,
            ],
            results_name="times",
            folder_path_latex=folder_path_export_latex,
            folder_path_csv=folder_path_export_csv,
        ...
        _results_to_pandas(
            list_df_results=[
                ...
                results_signal_analysis_user_profiles.df_hyper_params,
            ],
